...
description: >
  ...
  "technical_fallback"; Agent-Antworten werden scheinbar grundlos durch
  generische Fallback-Texte ersetzt (guardrail_triggered); CORS-Fehler im
  Browser; oder jemand will WebSockets (wieder) einführen. Schlüsselwörter:
...
| D | Client bekommt 429 | IMMER der eigene In-Process-Rate-Limiter (pro Worker!) | Response-Detail "Zu viele Anfragen" + `Retry-After`-Header = eigener Limiter | Abschnitt D |
...
| F | Score 0, `evaluation_status: "technical_fallback"` | Judge-LLM lieferte kein valides JSON, auch Repair-Call scheiterte | Logs `evaluation_json_parse_failed` / `evaluation_json_repair_failed` | Abschnitt F |
...
## C. 401/503 — die drei Auth-Schichten

Es gibt **drei getrennte** Auth-Mechanismen. Verwechslung ist die häufigste
Fehlbedienung.
...
## D. 429 — eigener Rate-Limiter vs. OpenRouter

**Entscheidungsregel: Ein 429, das der Client sieht, kommt IMMER vom eigenen
Rate-Limiter.** OpenRouter-429er werden vom OpenAI-SDK intern retried
...
mit `type=<ExceptionKlasse>`, z. B. `RateLimitError`).
...
Limits (definiert als Dependencies in `backend/api/routes.py`, Stand 2026-07-08):
...
1. Judge-Call (max_tokens 1200) muss reines JSON liefern.
...
`backend/agents/orchestrator.py`), nicht umformuliert.
...
- Das Feld `websocket_url` in `SessionResponse`
  (`backend/api/routes.py`) ist ein **totes Relikt** — es gibt keinen
...
Python-Repro-Kommandos wurden ausgeführt. Re-Verifikation
drift-anfälliger Fakten:
...
| Diagnostics-Felder | `sed -n '140,159p' backend/main.py` |
