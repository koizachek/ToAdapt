...
| C | 403 "Nur für den Master-Tutor" auf `briefings/upload`, `/api/teacher/upload-token`, `/briefings/batches` oder `/briefings/{id}/assessment` (seit 2026-09-02) | Tutor-Session ohne Master-Flag — Upload, Batch-Status, Zuordnung und interne Einstufung sind dem Login mit dem Master-Code (`TEACHER_ARCHIVE_CODE`) vorbehalten | Mit dem Master-Code neu einloggen; die Seite `/briefings` zeigt dann den Canvas-Upload | GEWOLLT, kein Bug |
| C | 503 "In der Pilotphase ist nur die Tutorenansicht freigeschaltet" auf `/sessions`, `/tp`, `/admin/cases/generate` | `PILOT_TUTOR_ONLY=1` (Railway) — gewollt für die Pilotphase HS26 | Zum Freischalten der Studierenden `PILOT_TUTOR_ONLY` entfernen UND `NEXT_PUBLIC_PILOT_TUTOR_ONLY=0` auf Vercel setzen | GEWOLLT |
| C | Tutor landet nach Login auf `/briefings`, `/cases` oder `/admin` leiten um | Pilotmodus im Frontend (`NEXT_PUBLIC_PILOT_TUTOR_ONLY` ≠ `0`) | s.o. | GEWOLLT |
| C | ÜGL sieht auf `/briefings` keine Briefings, Master sieht sie | Tutor-Kennung ist keine Übungsgruppe (`X-Teacher-Id` muss als `UEG07`/`7` parsebar sein) | `TEACHER_ACCESS_CODES`-Schlüssel prüfen (Konvention: Kennung nennt die Übungsgruppe(n), `UEG07` oder `UEG07+UEG12`) | Konfiguration, kein Bug |
| C | 401 "Upload-Token ungültig/abgelaufen" beim Direkt-Upload | Token älter als 15 min, `TOADAPT_API_KEY` auf Vercel ≠ Railway, oder Session nach Logout widerrufen | Seite neu laden (neues Token), Keys vergleichen | Abschnitt C |
| C | Upload aus dem Browser scheitert mit CORS-Fehler | Direkt-Upload geht an Railway, nicht über den Proxy — `ALLOWED_ORIGINS` enthält die Vercel-Domain nicht | `ALLOWED_ORIGINS` in Railway setzen | Abschnitt C |
| C | 413 beim Upload über `/api/teacher/...` | Vercel-Body-Limit 4,5 MB (Infrastruktur) — Upload muss den Direkt-Pfad (Upload-Token) nutzen, nie den Proxy | Frontend `/briefings` verwenden (macht das automatisch) | GEWOLLT |
| C | 423 "Feedback erst nach dem Termin" auf `/briefings/.../feedback` | `FEEDBACK_RELEASE_GATE=1` gesetzt und Freigabedatum (Tag nach dem Termin, `BRIEFING_SCHEDULE`) noch nicht erreicht — Standard ist die Sperre AUS | Warten, Master `force=1` (geloggt), oder Variable entfernen | GEWOLLT, wenn eingeschaltet |
| C | Batch bleibt `running`, Fortschritt steht, `stale=true` | Worker-Prozess starb mitten im Hintergrund-Task (Redeploy/Restart) | ZIP erneut hochladen — neuester Datensatz je Stammgruppe gewinnt | Bewusste Einschränkung (keine Job-Queue) |
...
| jti-Sperrliste (`backend/auth.py::reject_revoked_teacher_session`, seit 2026-07-17) | Proxy-Header `X-Teacher-Session` (jti aus dem Cookie) gegen Collection `revoked_teacher_sessions` | Dashboard-/Admin-/Briefings-Router (nur Requests MIT dem Header; Skripte mit reinem API-Key sind nicht betroffen) | Kein Effekt (fail-open — Härtung on top der 12-h-Frist) | **401** "Sitzung wurde abgemeldet — bitte neu einloggen" (nach Logout; erneuter Login erzeugt neue jti) |
