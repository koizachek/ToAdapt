...
| C | ÜGL sieht auf `/briefings` keine Briefings, Master sieht sie | Tutor-Kennung ist keine Übungsgruppe (`X-Teacher-Id` muss als `UEG07`/`7` parsebar sein) | `TEACHER_ACCESS_CODES`-Schlüssel prüfen (Konvention: Kennung = Übungsgruppe) | Konfiguration, kein Bug |
